...
python_config = script_dir.parent.parent / ".config" / "python"
venv_python = python_config / "venv-hoarder-tools" / "bin" / "python3"
...
def process_album_folder(folder, dry_run=False):
    ...
    if should_replace_cover(cover_path):
        ...
        if audio_file:
            ...
            try:
                # Check if COVIT is available in the centralized bin directory
                script_dir = Path(__file__).parent
                scripts_root = script_dir.parent.parent.parent  # Go up to ~/Scripts
                covit_path = scripts_root / ".config" / "python" / "bin" / "covit"
